...
- **Real-time WebSocket** — subscribe to messages via WebSocket as alternative to webhooks
- **Rate limiting** — 100 req/min per IP on bridge API
- **Input sanitization** — auto-sanitized sender IDs and room IDs with SHA-256 fallback
...
| [Encryption](docs/encryption.md) | E2BE setup, crypto architecture, per-user mode |
...
| [加密](docs/encryption.md) | E2BE 设置、加密架构、per-user 模式 |
...
└── appservice/    # HTTP server, dispatcher, puppet manager, crypto, WebSocket, auth
