...
# 同时也用于 WebSocket 连接的认证（连接后首条消息需发送
# {"access_token":"<api_key>"} ）。
# 未设置时（默认），Bridge API 和 WebSocket 均无需认证——
# 适用于内部/可信网络部署。
...
# Matrix 用户自动邀请列表。
# 当 Bridge 自动创建房间时，这些用户会被自动邀请加入。
# 不设置时，自动创建的房间仅包含 bridge bot。
# auto_invite = ["@admin:im.fr.ds.cc"]

# 是否允许 Bridge API 中房间创建请求的 `invite` 字段。
# 当 false（默认）时，`invite` 字段会被静默忽略——
# 仅使用上方 auto_invite 配置中的用户列表。
# 这可以防止外部服务邀请任意 Matrix 用户。
# allow_api_invite = false

# 是否允许跨平台中继（relay）。
# 当 false（默认）时，仅转发真实 Matrix 用户的消息到外部平台。
# 当 true 时，来自其他外部平台（如 telegram -> discord）的消息
# 也可以通过 Matrix 作为中枢进行转发。
# 每个 webhook/WebSocket 的 forward_sources 白名单控制
# 哪些跨平台消息会被中继。
# allow_relay = false
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `auto_invite` | string 数组 | `[]` | Bridge 自动创建房间时邀请的 Matrix 用户列表 |
| `allow_api_invite` | bool | `false` | 是否允许 API 请求中的 `invite` 字段生效 |
| `allow_relay` | bool | `false` | 是否允许跨平台中继（telegram -> discord 等） |
| `api_key` | string | _(空)_ | Bridge API 和 WebSocket 的认证密钥 |
| `webhook_ssrf_protection` | bool | `false` | 是否启用 Webhook URL 的 SSRF 防护 |
...
1. **加载配置** -- 读取 `BRIDGE_CONFIG` 指定的配置文件，校验所有字段（包括验证 `auto_invite` 中的用户 ID 格式）。
...
6. **启动 HTTP 监听** -- 在配置的地址和端口上开始接收请求（包括 WebSocket 端点）。
...
- `auto_invite` 中的用户 ID 格式是否正确（必须以 `@` 开头且包含 `:`）。
