...
Platform Spaces, bot commands, capabilities, permissions, and code quality improvements.

**New Features:**
- **Platform Spaces** — auto-creates a Matrix Space per platform; rooms are added as children. Falls back gracefully when the homeserver doesn't support Spaces (detected via error response, disabled for session lifetime)
- **Bot DM commands** — `!help`, `!platforms`, `!rooms`, `!spaces`, `!<platform>` (show details), `!<platform> <cmd>` (passthrough to external platform via webhook/WS)
- **Platform capabilities** — integrations declare supported features (`message`, `image`, `reaction`, `command`, etc.) during webhook registration or WS connection; queryable via `!<platform>` and `GET /api/v1/admin/capabilities`
- **WebSocket capabilities** — WS clients can declare capabilities via `?capabilities=` query param
- **Webhook owner** — `owner` field on webhooks for per-platform auto-invite into portal rooms
- **Admin/relay permission model** — single `[permissions]` config with `admin` and `relay` lists; admin = full access + invite, relay = message forwarding only
- **Room power level filtering** — `relay_min_power_level` config controls minimum room power level for message forwarding
- **Space admin endpoints** — `GET /api/v1/admin/spaces`, `GET /api/v1/admin/capabilities`

**Code Quality (pma-rust audit):**
- Added `rust-toolchain.toml` (pinned 1.88.0)
- Secret Debug redaction — `AppserviceConfig` and `EncryptionConfig` custom `Debug` impls hide `as_token`, `hs_token`, `api_key`, `crypto_store_passphrase`
- Eliminated all `.expect()` in non-test code — `Dispatcher::new`, `shutdown_signal`, `generate_token` now return `Result`
- CI uses `cargo cranky` instead of `cargo clippy` for stricter linting
- Added unified `just check` recipe (fmt-check → cranky → deny → test → build-release)
- Added `AppConfig::validate()` with systematic startup validation
- Extracted `MAX_MEDIA_SIZE` constant to core crate (was duplicated 3x)
- Extracted `PaginationParams` helper (was duplicated 4x across handlers)
- Extracted `is_valid_matrix_user_id()` to core (was duplicated 2x)

**Database:**
- Consolidated all migrations into single `001_initial.sql` (dev stage, no migration compat needed)
- Added `platform_spaces` table
- Added `capabilities` and `owner` columns to `webhooks` table

**Documentation:**
- Split integration guide into EN (`integration-guide.md`) and ZH (`integration-guide.zh.md`)
- Added complete examples: webhook receiver, WebSocket client, updated chat UI
- Updated README with new features, permissions section, and corrected doc links
- Added `encryption.zh.md` Chinese encryption guide

**Tests:** 157+ tests passing across all workspace crates.
