...
impl MatrixClient {
    /// Send a plain text message to a room as the bridge bot itself.
    ///
    /// Used for bot command responses in DM rooms.
    pub async fn send_text_as_bot(&self, room_id: &str, body: &str) -> anyhow::Result<String> {
        ...
        let resp = Self::check_response(resp, "send text as bot").await?;
