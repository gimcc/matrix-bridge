...
impl MatrixClient {
    ...
    /// Clear the display name for a puppet user (set to empty string).
    pub async fn clear_display_name(&self, user_id: &str) -> anyhow::Result<()> {
        let url = format!(
            "{}/_matrix/client/v3/profile/{}/displayname",
            self.homeserver_url,
            urlencoding::encode(user_id)
        ...
        let resp = self
            ...
            .bearer_auth(self.as_token.expose_secret())
            .query(&[("user_id", user_id)])
            .json(&json!({ "displayname": Value::Null }))
            .send()
            .await?;

        if !resp.status().is_success() {
            let text = resp.text().await?;
            warn!(user_id, "failed to clear display name: {text}");
        }
        Ok(())
    }
    ...
    /// Clear the avatar for a puppet user.
    pub async fn clear_avatar(&self, user_id: &str) -> anyhow::Result<()> {
        let url = format!(
            "{}/_matrix/client/v3/profile/{}/avatar_url",
            self.homeserver_url,
            urlencoding::encode(user_id)
        ...
        let resp = self
            ...
            .bearer_auth(self.as_token.expose_secret())
            .query(&[("user_id", user_id)])
            .json(&json!({ "avatar_url": Value::Null }))
            .send()
            .await?;

        if !resp.status().is_success() {
            let text = resp.text().await?;
            warn!(user_id, "failed to clear avatar: {text}");
        }
        Ok(())
    }
}
