...
mod tests {
    ...
    fn deserialize_send_message_text() {
        ...
        assert!(matches!(req.content, ContentPayload::Text { body, html } if body == "Hello!" && html.is_none()));
    }
    ...
    fn deserialize_send_message_image() {
        ...
        let req: SendMessageRequest = serde_json::from_value(json).unwrap();
        assert!(matches!(req.content, ContentPayload::Image { url, mimetype, .. } if url == "mxc://example.com/abc" && mimetype == "image/png"));
    }
    ...
    fn deserialize_send_message_file() {
        ...
        let req: SendMessageRequest = serde_json::from_value(json).unwrap();
        assert!(matches!(req.content, ContentPayload::File { filename, mimetype, .. } if filename == "doc.pdf" && mimetype == "application/octet-stream"));
    }
    ...
    fn deserialize_send_message_location() {
        ...
        let req: SendMessageRequest = serde_json::from_value(json).unwrap();
        assert!(matches!(req.content, ContentPayload::Location { latitude, longitude } if (latitude - 48.8566).abs() < 0.001 && (longitude - 2.3522).abs() < 0.001));
    }
    ...
    fn deserialize_send_message_reaction() {
        ...
        let req: SendMessageRequest = serde_json::from_value(json).unwrap();
        assert!(matches!(req.content, ContentPayload::Reaction { target_id, emoji } if target_id == "msg_100" && emoji == "thumbsup"));
    }
    ...
    fn deserialize_send_message_edit() {
        ...
        let req: SendMessageRequest = serde_json::from_value(json).unwrap();
        assert!(matches!(req.content, ContentPayload::Edit { target_id, .. } if target_id == "msg_50"));
    }
    ...
    fn convert_content_text() {
        let result = convert_content(ContentPayload::Text { body: "hello".into(), html: Some("<b>hello</b>".into()) });
        assert!(matches!(result, MessageContent::Text { body, formatted_body } if body == "hello" && formatted_body.as_deref() == Some("<b>hello</b>")));
    }
    ...
    fn convert_content_emote() {
        let result = convert_content(ContentPayload::Emote { body: "dances".into() });
        assert!(matches!(result, MessageContent::Emote { body } if body == "dances"));
    }

    #[test]
    fn convert_content_reaction() {
        let result = convert_content(ContentPayload::Reaction { target_id: "m1".into(), emoji: "heart".into() });
        assert!(matches!(result, MessageContent::Reaction { target_id, emoji } if target_id == "m1" && emoji == "heart"));
    }

    #[test]
    fn convert_content_redaction() {
        let result = convert_content(ContentPayload::Redaction { target_id: "m2".into() });
    ...
    fn convert_content_edit_recursive() {
        let result = convert_content(ContentPayload::Edit {
            target_id: "m3".into(),
            new_content: Box::new(ContentPayload::Text { body: "fixed".into(), html: None }),
        });
    ...
    fn convert_content_location() {
        let result = convert_content(ContentPayload::Location { latitude: 40.7, longitude: -74.0 });
        assert!(matches!(result, MessageContent::Location { latitude, longitude } if (latitude - 40.7).abs() < 0.01 && (longitude - (-74.0)).abs() < 0.01));
    }
