...
async fn handler_create_webhook_valid() {
    let (app, _) = build_test_app().await;
    let req = json_post("/api/v1/webhooks", json!({ "platform": "telegram", "url": "https://hooks.example.com/tg" }));
    let (status, body) = send_request(app, req).await;
    assert_eq!(status, StatusCode::CREATED);
    assert!(body["id"].is_number());
}
...
async fn handler_create_webhook_ssrf_blocked() {
    let (app, _) = build_test_app().await;
    let req = json_post("/api/v1/webhooks", json!({ "platform": "telegram", "url": "http://127.0.0.1/evil" }));
    let (status, body) = send_request(app, req).await;
...
async fn handler_create_webhook_invalid_scheme() {
    let (app, _) = build_test_app().await;
    let req = json_post("/api/v1/webhooks", json!({ "platform": "telegram", "url": "ftp://files.example.com/hook" }));
    let (status, body) = send_request(app, req).await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    assert!(body["error"].as_str().unwrap().contains("unsupported scheme"));
}
...
async fn handler_webhook_crud_cycle() {
    ...
    let app2 = build_bridge_api_router().with_state(state.clone());
    let (status, body) = send_request(app2, get_req("/api/v1/admin/webhooks?platform=discord")).await;
    assert_eq!(status, StatusCode::OK);
    ...
    let app4 = build_bridge_api_router().with_state(state);
    let (status, body) = send_request(app4, get_req("/api/v1/admin/webhooks?platform=discord")).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["webhooks"], json!([]));
}
...
async fn handler_room_name_too_long() {
    ...
    let (status, body) = send_request(app, req).await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    assert!(body["error"].as_str().unwrap().contains("room_name exceeds 255"));
}
