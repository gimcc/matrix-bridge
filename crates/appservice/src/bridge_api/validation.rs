use crate::dns_resolver::is_private_ip;
...
mod tests {
    ...
    async fn validate_webhook_url_accepts_valid_http() {
        assert!(
            validate_webhook_url("http://example.com/webhook", true)
                .await
                .is_ok()
        );
        assert!(
            validate_webhook_url("https://example.com/webhook", true)
                .await
                .is_ok()
        );
        assert!(
            validate_webhook_url("https://hooks.slack.com/services/abc", false)
                .await
                .is_ok()
        );
    }
    ...
    async fn validate_webhook_url_rejects_non_http_schemes() {
        let err = validate_webhook_url("ftp://example.com/file", true)
            .await
            .unwrap_err();
        assert!(err.contains("unsupported scheme"), "got: {err}");
        let err = validate_webhook_url("file:///etc/passwd", true)
            .await
            .unwrap_err();
        assert!(err.contains("unsupported scheme"), "got: {err}");
        let err = validate_webhook_url("javascript:alert(1)", false)
            .await
            .unwrap_err();
        assert!(err.contains("unsupported scheme"), "got: {err}");
    }
    ...
    async fn validate_webhook_url_rejects_private_ips_with_ssrf_on() {
        let err = validate_webhook_url("http://127.0.0.1/hook", true)
            .await
            .unwrap_err();
        assert!(err.contains("blocked"), "got: {err}");
        let err = validate_webhook_url("http://10.0.0.1/hook", true)
            .await
            .unwrap_err();
        assert!(err.contains("blocked"), "got: {err}");
        let err = validate_webhook_url("http://172.16.0.1/hook", true)
            .await
            .unwrap_err();
        assert!(err.contains("blocked"), "got: {err}");
        let err = validate_webhook_url("http://192.168.1.1/hook", true)
            .await
            .unwrap_err();
        assert!(err.contains("blocked"), "got: {err}");
        let err = validate_webhook_url("http://169.254.1.1/hook", true)
            .await
            .unwrap_err();
        assert!(err.contains("blocked"), "got: {err}");
        let err = validate_webhook_url("http://0.0.0.0/hook", true)
            .await
            .unwrap_err();
        assert!(err.contains("blocked"), "got: {err}");
        let err = validate_webhook_url("http://100.64.0.1/hook", true)
            .await
            .unwrap_err();
        assert!(err.contains("blocked"), "got: {err}");
        let err = validate_webhook_url("http://localhost/hook", true)
            .await
            .unwrap_err();
        assert!(err.contains("blocked"), "got: {err}");
        let err = validate_webhook_url("http://metadata.google.internal/v1/metadata", true)
            .await
            .unwrap_err();
        assert!(err.contains("blocked"), "got: {err}");
    }
    ...
    async fn validate_webhook_url_rejects_ipv6_loopback_with_ssrf_on() {
        let err = validate_webhook_url("http://[::1]/hook", true)
            .await
            .unwrap_err();
        assert!(err.contains("blocked"), "got: {err}");
    }
    ...
    async fn validate_webhook_url_accepts_private_ips_with_ssrf_off() {
        assert!(
            validate_webhook_url("http://127.0.0.1/hook", false)
                .await
                .is_ok()
        );
        assert!(
            validate_webhook_url("http://10.0.0.1/hook", false)
                .await
                .is_ok()
        );
        assert!(
            validate_webhook_url("http://192.168.1.1/hook", false)
                .await
                .is_ok()
        );
        assert!(
            validate_webhook_url("http://172.16.0.1/hook", false)
                .await
                .is_ok()
        );
        assert!(
            validate_webhook_url("http://localhost/hook", false)
                .await
                .is_ok()
        );
        assert!(
            validate_webhook_url("http://[::1]/hook", false)
                .await
                .is_ok()
        );
    }
