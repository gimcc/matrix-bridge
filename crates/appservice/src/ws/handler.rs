...
pub async fn handle_ws_upgrade(
    ...
    let (platform, forward_sources) = match validate_ws_params(&params) {
        Ok(v) => v,
...
async fn handle_ws_session(
    ...
    let (client_id, mut rx) = match state
        .ws_registry
        .try_register(&platform, forward_sources, MAX_WS_CLIENTS)
    {
        Ok(v) => v,
        Err(()) => {
...
pub(super) mod validation_tests {
    ...
    fn rejects_when_connection_limit_reached() {
        ...
        let _rxs: Vec<_> = (0..MAX_WS_CLIENTS)
            .map(|i| {
                let (_id, rx) = registry
                    .try_register(&format!("p{i}"), vec![], MAX_WS_CLIENTS)
                    .unwrap();
                rx
            })
            .collect();
        ...
        assert!(registry
            .try_register("telegram", vec![], MAX_WS_CLIENTS)
            .is_err());
