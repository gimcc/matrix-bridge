...
impl WsRegistry {
    ...
    /// Check if any WebSocket clients are connected for a given platform.
    pub fn has_clients(&self, platform_id: &str) -> bool {
        self.clients.get(platform_id).is_some_and(|v| !v.is_empty())
    }

    /// List all platform IDs that have connected WS clients.
    ...
    /// Get aggregated capabilities from all WS clients for a given platform.
    pub fn get_capabilities(&self, platform_id: &str) -> Vec<String> {
        let mut caps = std::collections::BTreeSet::new();
        if let Some(clients) = self.clients.get(platform_id) {
