...
impl PuppetManager {
    ...
    ///
    /// The cache key uses the localpart (already normalized/lowercased) so that
    /// case-variant external IDs mapping to the same Matrix user are deduplicated.
    pub async fn ensure_puppet_direct(
        ...
        let cache_key = localpart.to_string();
        ...
        if let Some(entry) = self.cache.get(&cache_key) {
            ...
            if name_changed {
                match display_name {
                    Some(name) => self.matrix_client.set_display_name(user_id, name).await?,
                    None => self.matrix_client.clear_display_name(user_id).await?,
                }
            }
            if avatar_changed {
                match avatar_url {
                    Some(url) if is_valid_avatar_url(url) => {
                        self.matrix_client.set_avatar(user_id, url).await?;
                    }
                    Some(url) => {
                        warn!(
                            url,
                            "rejected invalid avatar URL (only mxc:// and https:// allowed)"
                        );
                    }
                    None => self.matrix_client.clear_avatar(user_id).await?,
                }
            }
            self.db
                .upsert_puppet(
        ...
        // Look up by the expected matrix_user_id (derived from the normalized
        // localpart) rather than by external_user_id.  This ensures case-variant
        // external IDs that normalise to the same localpart resolve to the
        // existing puppet row instead of creating a conflicting one.
        let expected_matrix_user_id =
            format!("@{}:{}", localpart, self.matrix_client.server_name());
        let existing = self
            .db
            .find_puppet_by_matrix_id(&expected_matrix_user_id)
            .await?;
        ...
        let matrix_user_id = if let Some(puppet) = existing {
            ...
            if name_changed {
                match display_name {
                    ...
                    None => {
                        self.matrix_client
                            .clear_display_name(&puppet.matrix_user_id)
                            .await?;
                    }
                }
            }
            if avatar_changed {
                match avatar_url {
                    ...
                    None => {
                        self.matrix_client
                            .clear_avatar(&puppet.matrix_user_id)
                            .await?;
                    }
                }
            }
