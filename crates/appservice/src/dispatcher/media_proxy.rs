...
/// Maximum file size we will download from an external URL (200 MB).
    ...
    async fn download_and_upload_inner(
        ...
        if let Some(len) = resp.content_length() {
            if len > MAX_DOWNLOAD_SIZE {
                anyhow::bail!(
                    "file too large: Content-Length {len} exceeds {MAX_DOWNLOAD_SIZE} byte limit"
                );
            }
        }
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk?;
            if data.len() + chunk.len() > MAX_DOWNLOAD_SIZE as usize {
                anyhow::bail!(
                    "file too large: exceeded {MAX_DOWNLOAD_SIZE} byte limit during download"
                );
            }
