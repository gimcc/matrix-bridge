...
impl Dispatcher {
    ...
    pub async fn handle_incoming_http(
        ...
        let room_mapping = match room_mapping {
            Some(m) => m,
            None => {
                let room_name = message.room.name.as_deref().unwrap_or(&external_room_id);
                ...
                // Collect invite list: global auto_invite + platform webhook owners.
                let mut invite_list = self.auto_invite.clone();
                if let Ok(owners) = self.db.get_platform_owners(platform_id).await {
                    for owner in owners {
                        if !invite_list.contains(&owner) {
                            invite_list.push(owner);
                        }
                    }
                }

                // Refuse to create a zombie room with only bot + puppets.
                if invite_list.is_empty() {
                    return Err(BridgeError::Matrix(format!(
                        "no real Matrix users to invite for platform '{platform_id}': \
                         set auto_invite in config or webhook owner via API"
                    )));
                }

                let invite_refs: Vec<&str> = invite_list.iter().map(|s| s.as_str()).collect();
                let matrix_room_id = self
                    .matrix_client
                    .create_room(Some(room_name), &invite_refs, self.encryption_default)
                    .await
                    .map_err(|e| {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        ...
                        let _ = self.matrix_client.leave_room_as_bot(&matrix_room_id).await;
                ...
                self.ensure_platform_space(platform_id, &matrix_room_id)
                    .await;
        ...
        let localpart =
            platform::puppet_localpart(&self.puppet_prefix, platform_id, &external_sender_id);
        ...
        // Idempotency: if this external message was already bridged, return the
        // existing Matrix event ID instead of creating a duplicate.
        if let Ok(Some(existing)) = self
            .db
            .find_message_by_external_id(platform_id, &message.id)
            .await
        {
            debug!(
                event_id = existing.matrix_event_id,
                platform = platform_id,
                external_id = message.id,
                "duplicate external message — returning existing mapping"
            );
            return Ok(existing.matrix_event_id);
        }
...
fn apply_media_url(content: &mut Value, url: &str, encrypted_file: Option<&EncryptedAttachment>) {
