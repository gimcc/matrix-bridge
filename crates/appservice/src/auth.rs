...
///
/// Used for API key authentication where query parameter support is
/// intentionally excluded to avoid token leakage in URLs/logs.
fn extract_bearer_token_header_only(request: &Request) -> Option<String> {
    request
        .headers()
        .get("authorization")
...
/// 2. `access_token={token}` query parameter (legacy, for hs_token only)
fn extract_bearer_token(request: &Request) -> Option<String> {
    let from_header = extract_bearer_token_header_only(request);
...
///
/// Both inputs are hashed with SHA-256 before comparison, so the ct_eq
/// always operates on fixed-size (32-byte) digests. This avoids leaking
/// the expected token's length through timing side-channels.
...
pub async fn verify_hs_token(request: Request, next: Next) -> Result<Response, StatusCode> {
    ...
    match extract_bearer_token(&request) {
        Some(t) if verify_token(&t, expected.expose_secret()) => Ok(next.run(request).await),
        Some(_) => {
...
pub async fn verify_api_key(request: Request, next: Next) -> Result<Response, StatusCode> {
    ...
    match extract_bearer_token_header_only(&request) {
        Some(t) if verify_token(&t, expected.expose_secret()) => Ok(next.run(request).await),
        Some(_) => {
...
#[derive(Clone)]
pub struct HsToken(pub SecretString);
